...
3. Run this script
4. Place calorimeter into cold water bath
5. Start timer
6. After 60 s remove calorimeter for cold water bath and place on absorbent material
7. After **90 s** of heating close the plot window
...
try:
    ...
    ax1.set_xlim(t_mean-293, t_mean-263)
    ax1.set_ylim(p_mean*0.92, p_mean*1.02 )
    ...
    ax2.set_xlim(0, 50)
    ax2.set_ylim(p_mean*0.92, p_mean*1.02 )
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Pressure (Pa)')
    ax2.set_title('Pressure vs Time')
    ax2.set_xticklabels([])
    
    fig.suptitle('Close plot window to finish data acquistion', fontsize=16, fontweight='bold',color='red')
...
start_idx = find_nearest(times, start_time) + 50
