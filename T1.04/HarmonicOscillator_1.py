...
rotation_speed = 20  # degrees per frame
...
for idx in range(20):  
    ...
    #cv2.circle(img, (cxr, cyr), 5, (255, 255, 255), -1)
...
mass = input("What mass are you using (g)? ")
...
#t_uniform = np.linspace(min(time), max(time), len(time)*1)
...
#ax1.plot(t_uniform, signal_uniform, 'g')
...
folder_path = os.path.join(os.getcwd(), 'Harmonic oscillator data', dateStamp)
os.makedirs(folder_path,exist_ok=True)

timeStamp = datetime.now().strftime('%H_%M_%S')
file_path1 = os.path.join(folder_path, timeStamp + '_mass_' + mass + 'g_Positions.txt')
file_path2 = os.path.join(folder_path, timeStamp + '_mass_' + mass + 'g_FFT.txt')
