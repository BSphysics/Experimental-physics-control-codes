...
volume_fn = np.poly1d([-0.6531 , 410.95]) # Calculates volume in mL from distance-to-volume calibration
...
try:
    ...
    print("Reading initial pressures and volumes to center plot...")
    initial_pressures = []
    initial_volumes = []
    for _ in range(10):  # try up to 10 cycles
        ...
        # Read distance line
            ...
            if floats:
                initial_volumes.append(np.mean(floats))
    ...
    if initial_volumes:
        initial_volume = volume_fn(np.mean(initial_volumes))
    else:
        print("Warning: No initial volume received, using default")
        initial_volume = 320
    ...
    pMin_centered = initial_pressure - 0.15*pressure_span
    pMax_centered = initial_pressure + (0.1 + mass/500)*pressure_span
    ...
    ax1.set_xlim(initial_volume-50, initial_volume+100)
...
print('\n Estimated work done (from area of PV plot = ' + str(np.round(area,4)) + ' J')


fig, ax = plt.subplots(1, 1)
scat = ax.scatter(V, p, c=p, s=12, cmap=cmap, norm=norm)
ax.set_xlim(initial_volume-50, initial_volume+100)
...
def on_click(event):
    ...
    if event.button == 1:  # left click
        ...
        if len(clicked_y) == 2:
            ...
            filename = os.path.join(folderName, 'Syringe position vs time plot.png')
