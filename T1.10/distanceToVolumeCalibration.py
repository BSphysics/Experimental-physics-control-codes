...
2. Open a valve to the atmosphere and lift the syringe up to 50 mL
3. Run this script
...
9. The script will do a linear fit that calibrates changes syringe position to changes in its volume
10. Use these values in the Thermo3.py script
11. Don't adjust the separation between the position sensor and the syringe otherwise this calibration will have to be re-done
...
ax1.set_ylabel("Distance (mm)", color='k' , fontsize = 20)
ax1.set_xlabel("Time (s)", color='k', fontsize = 20)
...
totalVolumes = 350 + deltaV

distances = np.asarray(clicked_d)
plt.close('all')
plt.figure()
plt.plot(distances, totalVolumes, 'mo')
plt.tick_params(axis='both', which='major', labelsize=20)
ax=plt.gca()
ax.set_ylabel("Volume (mL)", color='k' , fontsize = 20)
ax.set_xlabel("Distance (mm)", color='k', fontsize = 20)

linear_fit = np.polyfit(distances, totalVolumes, 1)
linear_fit_fn = np.poly1d(linear_fit)
d_s = np.arange(0,130,1)
plt.plot(d_s, linear_fit_fn(d_s), color = 'green')

filename = os.path.join(folderName,'distance to volume calibraton fit.png')
plt.savefig(filename)
plt.show()

print('Linear fit parameters = ' + str(np.round(linear_fit,3)))
