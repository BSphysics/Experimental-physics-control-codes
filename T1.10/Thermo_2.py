...
@author: bs426
This script is designed to measure the pressure as temperature changes (volume constant)
Volume is kept constant by isolating the glass syringe
Temperature changed by removing calorimeter from hot water bath

Aim: Continuously record data and plot T vs P graph at the end

Method:

1. Open manifold valve 1 to atmosphere
2. Turn syringe valve so that it is open to system
3. Boil kettle and add hot water to water bath.
4. Close manifold valve 1 to atomosphere
5. Run this script
6. Place calorimeter in hot water bath
7. Wait 60s or so for temp to stabilise (around 60 degrees) - note syringe plunger should rise
8. Close syringe valve to system
9. Remove calorimeter from hot water bath and allow to cool
10. Close plot window after 120 s of data recording
...
filename = os.path.join(folderName,'Raw pressure vs temperature plot.png')
...
stop_idx = find_nearest(times, start_time + 35)

TP_model = np.polyfit(T[start_idx : stop_idx] , p[start_idx : stop_idx] , 1)
#print('\n Temperature vs Pressure model estimate of absolute zero = ' + str(np.round(-TP_model[1] / TP_model[0])))

headers = "Time,Temperature,Pressure"
data = np.array([times[start_idx:] , T[start_idx:] , p[start_idx:]]).T
